// nnetbin/transform-nnet-posteriors.cc

// Copyright 2013-2014  The Johns Hopkins University (Author: Sri Harish Mallidi)
//           2016-2017  Uni Oldenburg & Excellence Cluster Hearing4all (Changes by: Angel Castro Martinez)
...
int main(int argc, char *argv[]) {
  try {
    ...
    const char *usage =
        ...
        "Read activations from forward pass through Neural Network and output phone (posterior) probabilities. \n"
        "It requires a two column file mapping pdf-id to the corresponding pseudo phone-id. \n"
        "\n"
        "Usage:  transform-nnet-posteriors [options] <pdf-to-pseudo-phone> <feature-rspecifier> <feature-wspecifier>\n"
        "e.g.: \n"
        " transform-nnet-posteriors pdf_to_pseudo_phone.txt nnet ark:pdf_posteriors.ark ark:phone_posteriors.ark\n";
    ...
    std::string pdf_to_phone_rxfilename = po.GetArg(1);
    ...
    for (; !feature_reader.Done(); feature_reader.Next()) {
      ...
      if (pdf_to_phone_rxfilename != "") {
        ...
        phone_post.Resize(mat.NumRows(), max_phone_id + 1, kSetZero);
